...
def merge_sublists_to_dict(data, key_column, merge_column):
    """
    ...
    :param key_column: Index of the column to use as the dictionary key.
    :param merge_column: Index of the column to merge (extend lists for).
    ...
    for sublist in data:
        if len(sublist) <= key_column or len(sublist) <= merge_column:
            raise ValueError(f"Each sublist must have at least {max(key_column, merge_column) + 1} columns.")
    # Check that the merge_column contains lists, if expecting to merge lists
    for sublist in data:
        if not isinstance(sublist[merge_column], list):
            raise ValueError(f"The merge_column (index {merge_column}) must contain lists to merge.")
    # Initialize a defaultdict to store merged results based on the key_column
    merged_data = defaultdict(list)

    for sublist in data:
        key = sublist[key_column]
        merged_data[key].extend(sublist[merge_column])  # Merge the elements of the specified merge_column
